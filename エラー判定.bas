...
Function 接種判定(年齢 As Variant, 接種日 As Variant, ワクチン名 As String, 回数 As Long, 前回接種日 As Variant, 前回年齢E文 As String) As Variant()
    ...
    '現行ワクチン
        ...
        Case "ダイチロナ（ＸＢＢ．１．５）": 接種判定 = XBBダイチロナ判定(年齢, 接種日, 回数, 前回接種日)
    '終了ワクチン
...
Function 間隔判定(接種日 As Variant, 前回接種日 As Variant, 設定値 As Long, 単位 As String) As String
    ...
    End Select
End Function
Function XBBダイチロナ判定(年齢 As Variant, 接種日 As Variant, 回数 As Long, 前回接種日 As Variant) As Variant()
    Select Case 接種日
        Case Is >= #12/4/2023# '接種開始
            Select Case 回数
                Case 3, 4, 5, 6, 7: XBBダイチロナ判定 = Array(年齢判定(年齢, 12, 0), 間隔判定(接種日, 前回接種日, 3, "月"))
                Case Else: XBBダイチロナ判定 = Array("", "法定外回数")
            End Select
        Case Else: XBBダイチロナ判定 = Array("", "法定外ワクチン")
    End Select
End Function
